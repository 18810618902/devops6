...
from  forms import  WorkOrderCreateForm, WorkOrderResultForm
...
class  WorkOrderApplyView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return  render(request, 'order/workorder_apply.html', {'assign_to_sa':assign_to_sa_list})
...
class WorkOrderDetailView(LoginRequiredMixin, DetailView):
    ...
    def  post(self, request, *args, **kwargs):
        ...
        if work_order.status  == 0:
            work_order.status = 1
            work_order.save()
            return  render(request, 'order/workorder_detail.html', {'work_order':work_order, 'msg': '您已经接受工单'})
